...
solve'' solverSettings = interpret . solve solverSettings satConj
...
interpret solution =
  ...
  in
    case theResult of
      Sat m     -> Sat { model = M.fromList (map (first ((IM.!) theLabelMap)) (M.toList m)) }
      Unsat     -> Unsat
      Unknown   -> Unknown
...
solveBool solverSettings = solve solverSettings (return . const SatConjTrue)

-- Main entry point after setting up the CNF. For SMT instances
solve :: (Show a, Ord a) =>
         SolverSettings -> ([Literal (a, IS.Key)] -> Writer [(LogLevel, String)] SatConjResult)
         -> CNF a -> (Sat IS.Key, IntMap a, [(LogLevel, String)])
solve initialSettings satConjCheck cnf =
  ...
  in case preprocess (initialSettings {initRandgen=g2}) cnf of
    Just (preprocessedProblem, theAI) -> (\(x,y) -> (x, labelMap preprocessedProblem, y)) $
      runWriter
       (evalStateT
        (evalContT (callCC $ \exit -> cdcl satConjCheck exit 0 (IM.fromList [(-1, exit)]) preprocessedProblem False))
...
preprocess initialSettings (CNF theClauses) =
  let 
      ...
      theProblem = Problem {
              ...
              , partialAssignment = IM.map S.findMin (IM.union positive negative)
              , decisionLevels    = IM.map (const 0) (IM.union positive negative)
              -- also possible: -- , unassignedVariables = sortBy (\v -> fmap (negate) (M.lookup v counts)) (IS.toList theUnassignedVariables)
              -- Shuffle randomly once, may be repeated at every level
              , unassignedVariables =
                  if shuffle initialSettings
                    then map fst $ sortBy snd (zip (IS.toList theUnassignedVariables) rands)
                    else IS.toList theUnassignedVariables
